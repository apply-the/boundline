...
use crate::workspace_fixture::{
    extract_trace_path, run_boundline_in, temp_fixture_workspace, terminal_text,
};

#[test]
fn fixture_vertical_slice_drives_a_failing_test_to_green() {
    let workspace = temp_fixture_workspace("boundline-cli-vertical-slice");
    let initial =
    ...
    let output = run_boundline_in(&workspace, &["run"]);
