...
fn session_native_runtime_path_runs_without_a_declarative_profile() {
    ...
    assert!(plan_text.contains("next_command: boundline run"), "{plan_text}");
...
fn status_surfaces_confirmed_and_skipped_flow_states() {
    ...
    assert_eq!(
        run_boundline_in(&confirmed_workspace, &["plan", "--flow", "change"]).status.code(),
        Some(0)
    );

    let confirmed_status = run_boundline_in(&confirmed_workspace, &["status"]);
    ...
    assert_eq!(run_boundline_in(&skipped_workspace, &["plan", "--no-flow"]).status.code(), Some(0));

    let skipped_status = run_boundline_in(&skipped_workspace, &["status"]);
