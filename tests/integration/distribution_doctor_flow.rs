...
fn doctor_install_reports_missing_canon_governance_operation() {
    let canon_dir =
        fake_canon_directory_with_capabilities("0.41.0", MISSING_OPERATION_CAPABILITIES);
    let output = Command::new(env!("CARGO_BIN_EXE_boundline"))
        .args(["doctor", "--install"])
        .env("PATH", &canon_dir)
        .current_dir(env!("CARGO_MANIFEST_DIR"))
...
fn doctor_install_reports_missing_canon_mode() {
    let canon_dir = fake_canon_directory_with_capabilities("0.41.0", MISSING_MODE_CAPABILITIES);
    let output = Command::new(env!("CARGO_BIN_EXE_boundline"))
        .args(["doctor", "--install"])
        .env("PATH", &canon_dir)
        .current_dir(env!("CARGO_MANIFEST_DIR"))
