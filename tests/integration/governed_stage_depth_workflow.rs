...
fn workflow_status_refreshes_governed_investigate_approval_before_resume() {
    ...
    assert!(
        status_text.contains("next_command: boundline preview workflow resume --workspace "),
        "{status_text}"
    );
    ...
    assert!(
        resume_text.contains("next_command: boundline preview workflow inspect --workspace "),
        "{resume_text}"
    );
