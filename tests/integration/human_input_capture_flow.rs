...
fn capture_with_brief_only_succeeds_and_persists_session() {
    ...
    let capture =
        run_boundline_in(&workspace, &["capture", "--brief", brief.to_string_lossy().as_ref()]);
...
fn capture_rejects_brief_outside_workspace() {
    ...
    run_boundline_in(&workspace, &["start"]);
    let capture =
        run_boundline_in(&workspace, &["capture", "--brief", brief.to_string_lossy().as_ref()]);
    let text = terminal_text(&capture);
...
fn capture_without_goal_or_brief_returns_clear_error() {
    ...
    let capture = run_boundline_in(&workspace, &["capture"]);
    let text = terminal_text(&capture);
    assert_eq!(capture.status.code(), Some(1), "{text}");
...
fn capture_records_clarification_for_unbounded_request_and_blocks_plan() {
    ...
    assert!(
        status_text.contains("next_command: boundline capture --goal <narrower goal>"),
        "{status_text}"
    );
    ...
    assert!(
        plan_text.contains("next_command: boundline capture --goal <narrower goal>"),
        "{plan_text}"
    );
}
