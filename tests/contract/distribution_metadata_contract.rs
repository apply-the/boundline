...
fn distribution_metadata_keeps_versions_and_bundle_names_aligned() {
    ...
    let winget_version =
        fs::read_to_string(repo_root.join(
            "distribution/winget/manifests/a/ApplyThe/Boundline/0.42.0/ApplyThe.Boundline.yaml",
        ))
        .unwrap();
    let winget_installer = fs::read_to_string(repo_root.join(
        "distribution/winget/manifests/a/ApplyThe/Boundline/0.42.0/ApplyThe.Boundline.installer.yaml",
    ))
    .unwrap();
    let winget_locale = fs::read_to_string(repo_root.join(
        "distribution/winget/manifests/a/ApplyThe/Boundline/0.42.0/ApplyThe.Boundline.locale.en-US.yaml",
    ))
    .unwrap();

    assert!(cargo_toml.contains("version = \"0.42.0\""));
    assert!(channel_metadata.contains("boundline_version = \"0.42.0\""));
    assert!(channel_metadata.contains("canon_version = \"0.40.0\""));
    ...
    assert!(formula.contains("version \"0.42.0\""));
    assert!(formula.contains("using: :git, tag: \"0.42.0\""));
    ...
    assert!(winget_version.contains("PackageVersion: 0.42.0"));
    assert!(winget_installer.contains("boundline-bundle-0.42.0-windows-x86_64.zip"));
    assert!(winget_installer.contains("releases/download/0.42.0/"));
