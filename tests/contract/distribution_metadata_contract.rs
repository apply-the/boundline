...
fn distribution_metadata_keeps_versions_and_bundle_names_aligned() {
    ...
    let winget_version =
        fs::read_to_string(repo_root.join(
            "distribution/winget/manifests/a/ApplyThe/Boundline/0.41.0/ApplyThe.Boundline.yaml",
        ))
        .unwrap();
    let winget_installer = fs::read_to_string(repo_root.join(
        "distribution/winget/manifests/a/ApplyThe/Boundline/0.41.0/ApplyThe.Boundline.installer.yaml",
    ))
    .unwrap();
    let winget_locale = fs::read_to_string(repo_root.join(
        "distribution/winget/manifests/a/ApplyThe/Boundline/0.41.0/ApplyThe.Boundline.locale.en-US.yaml",
    ))
    .unwrap();

    assert!(cargo_toml.contains("version = \"0.41.0\""));
    assert!(channel_metadata.contains("boundline_version = \"0.41.0\""));
