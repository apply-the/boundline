...
fn session_native_commands_accept_cluster_entrypoint() {
    ...
    match goal.command {
        Some(DeveloperCommand::Goal { cluster, goal, .. }) => {
    ...
    match plan.command {
        Some(DeveloperCommand::Plan { cluster, .. }) => {
            assert_eq!(cluster, Some(PathBuf::from("/tmp/primary")));
        }
    ...
    match status.command {
        Some(DeveloperCommand::Status { cluster, .. }) => {
            assert_eq!(cluster, Some(PathBuf::from("/tmp/primary")));
        }
...
fn session_run_preserves_workspace_requirement_for_custom_compatibility_mode() {
    ...
    .unwrap();
    let session =
        boundline::cli::DeveloperCommandSession::from_command(cli.command.as_ref().unwrap());
