# Adaptive Execution in Boundline 0.24.0
...
`<workspace>/.boundline/execution.json`, a compatibility execution profile can
describe an `adaptive` block and let Boundline choose one bounded workspace slice
...
`boundline init`; adaptive behavior is still configured through the execution
...
- adaptive evidence projected into `.boundline/session.json` and `.boundline/traces/`
- adaptive summaries surfaced in `boundline run`, `boundline status`, `boundline next`,
  and `boundline inspect`
...
configured, `read_targets` remain required because Boundline selects the slice from
that bounded set.

## How selection works

Boundline scores each `read_target` using only bounded local evidence:
...
When validation fails, Boundline can also derive bounded guidance from the latest
...
replacement pair. Boundline records those signatures and does not repeat the same
candidate after a failed validation.

Validation guidance changes ranking, not boundedness. Boundline still only
...
evidence for another materially different candidate, Boundline stops explicitly
...
- `boundline run`: explicit compatibility routing, `execution_condition`,
...
- `boundline status`: explicit `routing`, `execution_condition`,
...
- `boundline next`: the same adaptive session projection plus the CLI-reported next
  command, including inspect-only compatibility follow-up when no active
  session exists
- `boundline inspect`: adaptive slice-selection headlines, `adaptive_evidence`
