...
`/boundline:continue`. Copilot and Gemini are documented fallback paths rather
than claimed universal global command installs.

Host support modes stay explicit in `0.64.0`: Claude, Codex, and Copilot keep
repo-local full command-pack parity after workspace init; Cursor remains a
copy-ready bootstrap path; Gemini remains a manual CLI-first fallback. In all
...
### 3b. Optional Dashboard View

Use the dashboard when you want a terminal view over the same session-native
truth that powers `status`, `next`, and `inspect`:

```bash
boundline-dashboard --workspace <workspace>
```

For CI, contract checks, or non-interactive shells, ask for the typed snapshot
instead of starting the interactive screen:

```bash
boundline-dashboard --workspace <workspace> --snapshot-json
```

The normal CLI launcher is also available:

```bash
boundline dashboard --workspace <workspace>
```

The dashboard does not create workspace state. If a workspace is missing,
degraded, or not ready for interaction, it prints a valid fallback command such
as `boundline init`, `boundline start`, `boundline status`, `boundline inspect`,
...
boundline init \
  ...
  --owner platform \
  --assistant codex \
  --route planning=codex:gpt-5.3-codex \
  --route implementation=codex:gpt-5.3-codex
```
