...
If `boundline dashboard` reports the dedicated entrypoint unavailable, build or install the `boundline-dashboard` binary for the same Boundline `0.71.0` release, or continue with the normal CLI commands.
