...
product story. Canon should remain downstream from Boundline as governance and
...
This is the part that separates Boundline from a framework made of prompt files and
...
This is the part that makes Boundline more than prompt choreography.
...
This is why Boundline is already more than a Markdown framework while keeping the
...
This is part of what makes Boundline feel like orchestration rather than chat.
...
Keep the governance boundary where Boundline orchestrates and Canon governs.
...
- [src/adapters/governance_runtime.rs](../src/adapters/governance_runtime.rs) already models both local governance and Canon CLI integration as runtimes behind a Boundline-owned request/response boundary.
...
"make me a smart plan" LLM planner, Boundline becomes harder to trust, harder to
...
If Boundline does not own that loop explicitly, it risks becoming a static planner
with better wording instead of a real orchestrator.

### 6. Decision shape is not yet formalized enough

Even if Boundline starts choosing the next bounded action, that still is not enough
...
- [src/domain/flow.rs](../src/domain/flow.rs) already gives Boundline stage metadata and flow identity.
...
If the loop is not grounded in tool use, Boundline will regress into a planner that
mostly emits text.
...
The next spec should let Boundline consume Canon outputs as bounded planning inputs
...
- a first-class bounded decision object owned by Boundline, not hidden inside adapter-specific logic
...
3. Boundline proposes an inferred flow, shows it, and asks for lightweight confirmation unless the user already made an explicit choice.
...
9. Governance remains a Boundline-owned stage overlay backed by Canon artifacts and approvals, with Canon invoked at stage boundaries rather than per action.
...
It should ask, "How do we make Boundline feel like the orchestrator it already wants
