...
Boundline 0.73.0 ships the current planning-readiness chain. When the plan
...
If `plan_quality_state` or `backlog_quality_state` is
`clarification_required`, answer the one question in the emitted
