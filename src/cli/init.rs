...
mod tests {
    ...
    fn execute_init_blocks_when_canon_surface_is_unavailable() {
        ...
        let mut blocked_status = blocked_canon_install_status(
            ...
            "install or repair Canon 0.71.0 before rerunning init",
        );
        if let Some(surface) = blocked_status.surface_verification.as_mut() {
        ...
        assert!(
            report.terminal_output.contains("install or repair Canon 0.71.0 before rerunning init"),
            "{}",
            report.terminal_output
        );
    ...
    fn execute_init_fails_fast_before_planning_when_canon_surface_is_unavailable() {
        ...
        let mut blocked_status = blocked_canon_install_status(
            "Canon 0.10.0 is present but version 0.71.0 is required",
            "upgrade Canon to 0.71.0 or later",
        );
        if let Some(surface) = blocked_status.surface_verification.as_mut() {
        ...
        assert!(report.terminal_output.contains("0.71.0"), "{}", report.terminal_output);
