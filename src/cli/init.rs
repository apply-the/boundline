...
const OLLAMA_MODEL_NAMESPACE: &str = "ollama/";
const OLLAMA_SMALL_PROFILE_ID: &str = "ollama-small";
const OLLAMA_MEDIUM_PROFILE_ID: &str = "ollama-medium";
const OLLAMA_LARGE_PROFILE_ID: &str = "ollama-large";
const OLLAMA_SMALL_HARDWARE: &str = "Apple Silicon with 16 GB unified memory";
const OLLAMA_MEDIUM_HARDWARE: &str = "Apple Silicon or local workstation with 64 GB memory";
const OLLAMA_LARGE_HARDWARE: &str = "Apple Silicon or local workstation with 96/128 GB memory";
const QWEN_GENERAL_7B: &str = "qwen2.5:7b";
const QWEN_CODER_7B: &str = "qwen2.5-coder:7b";
const QWEN_GENERAL_14B: &str = "qwen2.5:14b";
const QWEN_CODER_14B: &str = "qwen2.5-coder:14b";
const QWEN_GENERAL_72B: &str = "qwen2.5:72b";
const QWEN_CODER_32B: &str = "qwen2.5-coder:32b";
...
/// Ollama model-routing presets generated by `boundline init`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, ValueEnum)]
#[value(rename_all = "kebab-case")]
pub enum OllamaProfile {
    /// Fits Apple Silicon laptops with 16 GB unified memory.
    Small,
    /// Targets a local workstation with enough memory for 14B class models.
    Medium,
    /// Targets high-memory unified-memory or workstation hosts for 32B/72B models.
    Large,
}
...
impl OllamaProfile {
    const fn profile_id(self) -> &'static str {
        match self {
            Self::Small => OLLAMA_SMALL_PROFILE_ID,
            Self::Medium => OLLAMA_MEDIUM_PROFILE_ID,
            Self::Large => OLLAMA_LARGE_PROFILE_ID,
        }
    }

    const fn hardware_summary(self) -> &'static str {
        match self {
            Self::Small => OLLAMA_SMALL_HARDWARE,
            Self::Medium => OLLAMA_MEDIUM_HARDWARE,
            Self::Large => OLLAMA_LARGE_HARDWARE,
        }
    }

    fn routes(self) -> Vec<(RouteSlot, ModelRoute)> {
        let (general, coder) = match self {
            Self::Small => (QWEN_GENERAL_7B, QWEN_CODER_7B),
            Self::Medium => (QWEN_GENERAL_14B, QWEN_CODER_14B),
            Self::Large => (QWEN_GENERAL_72B, QWEN_CODER_32B),
        };
        [
            (RouteSlot::Planning, coder),
            (RouteSlot::Implementation, coder),
            (RouteSlot::Verification, general),
            (RouteSlot::Review, general),
        ]
        .into_iter()
        .map(|(slot, model)| {
            (
                ...
                ModelRoute {
                    ...
                    model: format!("{OLLAMA_MODEL_NAMESPACE}{model}"),
                },
            )
        })
        .collect()
...
pub struct InitRequest<'a> {
    ...
    pub ollama_profile: Option<OllamaProfile>,
...
struct ResolvedInitInputs {
    ...
    ollama_profile_routes: Vec<(RouteSlot, ModelRoute)>,
...
fn resolve_init_inputs(
    ...
    let profile_routes = request.ollama_profile.map(OllamaProfile::routes).unwrap_or_default();
    let explicit_routes = request
        .routes
        .iter()
        .map(|raw_route| parse_model_route(raw_route))
        .collect::<Result<Vec<_>, _>>()?;
    let routes_for_prompting =
        if profile_routes.is_empty() { explicit_routes.clone() } else { profile_routes.clone() };
    let guided_prompt_state =
        guided_init_prompt_state(request, &stored_defaults, &routes_for_prompting);
    ...
    let guided_routes = if profile_routes.is_empty() {
        resolve_guided_routes(
            &explicit_routes,
    ...
    effective_routes.extend(profile_routes.iter().cloned());
    if explicit_routes.is_empty() {
        effective_routes.extend(guided_routes.clone());
    } else {
        effective_routes.extend(explicit_routes.clone());
    }
    ...
    Ok(ResolvedInitInputs {
        ...
        ollama_profile: request.ollama_profile,
        effective_assistants,
        ollama_profile_routes: profile_routes,
        explicit_routes,
        guided_routes,
        seeded_routes,
        effective_routes,
...
struct RouteSetupLineInput<'a> {
    catalog: &'a BundledModelCatalog,
    ollama_profile: Option<OllamaProfile>,
    ollama_profile_routes: &'a [(RouteSlot, ModelRoute)],
    effective_assistants: &'a [AssistantHostKind],
    guided_answers: Option<&'a GuidedInitAnswers>,
    explicit_routes: &'a [(RouteSlot, ModelRoute)],
    guided_routes: &'a [(RouteSlot, ModelRoute)],
    seeded_routes: &'a [SeededRouteSelection],
    inspect_command: String,
}

fn route_setup_lines(input: RouteSetupLineInput<'_>) -> Vec<String> {
    let mut lines = vec![format!("- catalog_source: {}", input.catalog.summary_label())];
    if let Some(profile) = input.ollama_profile {
        lines.push(format!(
            "- ollama_profile: {} ({})",
            profile.profile_id(),
            profile.hardware_summary()
        ));
        lines.extend(input.ollama_profile_routes.iter().map(|(slot, route)| {
            format!(
                "- preset {}: {}:{} [ollama-profile]",
                slot.as_str(),
                route.runtime,
                route.model
            )
    ...
    if input.effective_assistants.is_empty() {
        lines.push(
        ...
        lines.push(format!(
            "- assistant_defaults: {}",
            format_assistant_host_list(input.effective_assistants)
        ));
    }

    if let Some(answers) = input.guided_answers {
        lines.extend(
            answers.routes.iter().map(|selection| format!("- {}", selection.display_line().trim())),
        );
    } else {
        let explicit_route_lines =
            input.explicit_routes.iter().chain(input.guided_routes.iter()).collect::<Vec<_>>();
        if input.seeded_routes.is_empty()
            && explicit_route_lines.is_empty()
            && input.ollama_profile.is_none()
        {
            lines.push(
            ...
            lines.extend(input.seeded_routes.iter().map(|selection| {
    ...
    lines.push(format!("- inspect_or_edit: {}", input.inspect_command));
    lines
}
...
fn remove_boundline_auto_approve_entries(auto: &mut Map<String, Value>) {
    for key in [
        ...
        "/^boundline (doctor|status|inspect)\\b/",
        "/^boundline goal\\b/",
        "/^boundline plan\\b/",
        "/^boundline run\\b/",
        "/^boundline config show\\b/",
        "/^boundline workflow (list|status|inspect)\\b/",
        "/^boundline preview workflow (list|status|inspect)\\b/",
        ...
        "/^boundline preview (workflow (run|resume)|cluster init)\\b/",
        "/^boundline init\\b/",
        "/^boundline workflow (run|resume)\\b/",
        "/^boundline preview workflow (run|resume)\\b/",
        "/^boundline config (set|unset|bind-context|unbind-context)\\b/",
        "/^boundline cluster init\\b/",
        "/^boundline preview cluster init\\b/",
    ] {
        auto.remove(key);
    }
}
...
fn apply_read_only_auto_approve_entries(auto: &mut Map<String, Value>) {
    ...
    auto.insert(
        "/^boundline (init|run|config (set|unset|bind-context|unbind-context))\\b/".to_string(),
        auto_approve_rule(false),
    );
    auto.insert(
        "/^boundline preview (workflow (run|resume)|cluster init)\\b/".to_string(),
        auto_approve_rule(false),
    );
}
...
mod tests {
    ...
    use super::{
        ...
        InitRequest, OllamaProfile, POST_CHECKOUT_HOOK_NAME, POST_MERGE_HOOK_NAME,
        POST_REWRITE_HOOK_NAME, SEMANTIC_INDEX_HOOK_MANAGED_MARKER,
        SEMANTIC_INDEX_HOOK_TRIGGER_POST_MERGE, SEMANTIC_INDEX_HOOK_TRIGGER_POST_REWRITE,
        UpdateRequest, UpdateTarget, apply_requested_semantic_index_hook_action,
        canon_reviewer_route_readiness, collect_guided_init_answers_with_interactor,
        command_in_path, ensure_workspace_project_doc_roots, execute_init, execute_update,
        execution_template, format_runtime_list, format_slot_list, git_hooks_directory,
    ...
    #[test]
    fn ollama_profiles_pin_expected_stage_models() -> Result<(), Box<dyn std::error::Error>> {
        let small = OllamaProfile::Small.routes().into_iter().collect::<BTreeMap<_, _>>();
        let medium = OllamaProfile::Medium.routes().into_iter().collect::<BTreeMap<_, _>>();
        let large = OllamaProfile::Large.routes().into_iter().collect::<BTreeMap<_, _>>();

        assert_eq!(
            small
                .get(&RouteSlot::Planning)
                .ok_or_else(|| std::io::Error::other("missing small planning route"))?
                .model,
            "ollama/qwen2.5-coder:7b"
        );
        assert_eq!(
            small
                .get(&RouteSlot::Review)
                .ok_or_else(|| std::io::Error::other("missing small review route"))?
                .model,
            "ollama/qwen2.5:7b"
        );
        assert_eq!(
            medium
                .get(&RouteSlot::Implementation)
                .ok_or_else(|| std::io::Error::other("missing medium implementation route"))?
                .model,
            "ollama/qwen2.5-coder:14b"
        );
        assert_eq!(
            medium
                .get(&RouteSlot::Verification)
                .ok_or_else(|| std::io::Error::other("missing medium verification route"))?
                .model,
            "ollama/qwen2.5:14b"
        );
        assert_eq!(
            large
                .get(&RouteSlot::Implementation)
                .ok_or_else(|| std::io::Error::other("missing large implementation route"))?
                .model,
        ...
        assert_eq!(
            large
                .get(&RouteSlot::Review)
                .ok_or_else(|| std::io::Error::other("missing large review route"))?
                .model,
            "ollama/qwen2.5:72b"
        );
        assert!(large.values().all(|route| route.runtime == RuntimeKind::Codex));
        Ok(())
    }
