...
When inspect also reports `capability_provider_id`, `capability_provider_activation_state`, `capability_provider_validation_disposition`, `capability_provider_failure_class`, `capability_provider_accepted_evidence_refs`, `capability_provider_rejected_evidence_refs`, or `capability_provider_limitations`, preserve them exactly. Use those fields to explain provider readiness, permission, execution, or post-execution validation outcomes without inventing assistant-only state.
