...
handoffs:
  ...
  - label: Inspect Trace
    ...
    prompt: Inspect the latest trace
---
...
## Shell-Enabled Path
If the workspace is known, run `boundline run --workspace <workspace>` exactly once. If the host or wrapper needs the session-native event stream, treat `boundline orchestrate --workspace <workspace> --assistant-host copilot --until terminal --json-stream` as the internal backend mapping for the same continuation. Do not mention that backend mapping in user-facing next-step copy when `/boundline-run` or `boundline run` already expresses the same action. If the active session has no captured goal or planned task, route to `/boundline-plan` or `/boundline-goal` instead of inventing a new run command. If the user explicitly asks for direct manifest-backed compatibility behavior without relying on active session state, route them to the direct compatibility workflow in the shared guidance instead of reinterpreting `/boundline-run`.
...
When run output reports `completion_verification_state`, `completion_claim_kind`, `completion_claim_source`, `completion_blocked_claims`, `completion_evidence_refs`, `completion_verification_finding`, or `completion_verification_required_action`, preserve them exactly. If completion verification is anything other than `ready`, do not describe the run as successful. Surface the blocked claim, the proving command or rerun action, and the fact that Boundline owns proof execution while Canon may later consume the emitted evidence refs.
...
If completion verification is blocked, proof-required, stale, or failed, summarize the proving command or rerun action as the next concrete operator action and avoid success phrasing such as "done", "complete", "passed", or "finished".
When a run event carries `audit`, treat that object as the primary explanation of the execution step: preserve `audit.event`, `audit.algorithm`, `audit.actor`, `audit.outcome`, and `audit.message`. If `audit.actor.participant_routes` or `audit.actor.mixed_routes` is present, call out the multi-route council attribution explicitly instead of reporting a single synthetic route.
...
## Next-Step Routing
Prefer `assistant_resume_command` when a `phase_request` is present; otherwise prefer `assistant_next_command`; otherwise follow the CLI-reported `next_command`.
Render assistant-safe follow-up actions as Copilot command links or the defined handoff buttons instead of plain text shell guidance when a host route is available. For example, use `[Run /boundline-inspect](command:github.copilot.chat.execute?%5B%22%2Fboundline-inspect%22%5D)` when inspection is the next route.
Default to `/boundline-inspect` after a run completes. Route to `/boundline-run` only when delegation is pending or the workflow remains incomplete and another bounded continuation is appropriate. Route to `/boundline-plan` or `/boundline-goal` when the reported state says execution readiness is missing.
Allowed follow-up commands: `/boundline-inspect`, `/boundline-status`, `/boundline-run`, `/boundline-step`, `/boundline-plan`, `/boundline-goal`, `/boundline-govern`.
