...
Host support stays explicit in this release: Antigravity uses `repo-local-full` after init, Cursor remains `copy-ready-assets`, and global bootstrap for Antigravity remains `manual-fallback`. Do not imply richer parity for Cursor or global bootstrap than the declared support mode.
