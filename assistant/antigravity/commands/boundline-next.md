...
## Next-Step Routing
Prefer the CLI-reported `next_command`; if it points to inspect or `continuity_authority: compatibility_trace` is present, route to `/boundline-inspect`. Route to `/boundline-goal` only when the CLI reports no active session and no compatibility follow-up.
