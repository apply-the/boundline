...
When a user asks for a consolidated state view, prefer `boundline status --workspace <workspace>` for a compact summary or `boundline inspect --workspace <workspace>` for the detailed runtime projection. Repo-local assistant command packs generated by `boundline init --assistant <host>` should surface that same session-native truth in chat. Do not infer state from chat history or treat any assistant surface as a second workflow engine.
...
Canon-backed setup and config commands are also first-class assistant surfaces:
...
- `/boundline-goal`: Confirms the workspace and runs `cargo run --bin boundline -- orchestrate --workspace <workspace> --goal "<goal>" --until phase-request --json-stream` for direct goal text, `cargo run --bin boundline -- orchestrate --workspace <workspace> --brief <path> [--brief <path> ...] --until phase-request --json-stream` for brief-only capture, or the combined `--goal` plus repeated `--brief` variant when both are present. Treat this as the canonical assistant-host interactive path for goal capture and clarification. Direct `cargo run --bin boundline -- goal ...` remains the non-interactive capture primitive for raw shell workflows.
- `/boundline-plan`: Defines or refines the active session goal from direct goal text and/or supporting brief files, then runs the planning surface. When the user gives new goal text and/or supporting brief files, prefer `cargo run --bin boundline -- orchestrate --workspace <workspace> --goal "<goal>" --brief <path> [--brief <path> ...] --until phase-request --json-stream` so runtime-owned goal and planning gates can stop the flow. When the active session already has a captured goal and the user is only supplying planning input, use `cargo run --bin boundline -- plan --workspace <workspace> --input <path> --json`. Summaries should preserve proposed, confirmed, skipped, or absent flow state, `goal_plan_state`, `goal_plan_revision`, `planning_rationale`, `verification_strategy`, the negotiated delivery fields, and any CLI-reported confirm or clarification guidance.
...
If the user explicitly selects a built-in flow, assistants should run `cargo run --bin boundline -- flow <bug-fix|change|delivery> --workspace <workspace>` after recording the goal and before plan. There is no separate assistant command pack for `flow`; use the raw CLI subcommand directly.
