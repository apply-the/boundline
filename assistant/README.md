...
When the large-codebase context substrate is active, preserve
`repository_map_state`, `snapshot_cache_state`, `context_pack_entries`,
`omission_findings`, and `patch_safe_edit_attempts` exactly. Those fields
explain why Boundline compacted, omitted, downgraded, or blocked context before
...
The large-codebase substrate extends that same context projection further.
Preserve typed context-pack lines such as `repository_map_state`,
`snapshot_cache_state`, `context_pack_entries`, `omission_findings`, and
`patch_safe_edit_attempts` exactly when they appear on `plan`, `run`, `status`,
or `inspect`. They explain digest-backed compaction, archived-context
visibility, unsafe oversized full-read refusal, and patch-safe large-file edit
guardrails.
...
- Preserve `repository_map_state`, `snapshot_cache_state`, `context_pack_entries`, `omission_findings`, and `patch_safe_edit_attempts` across assistant turns once planning or follow-through reports them.
