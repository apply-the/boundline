...
Host support stays explicit in this release: Claude uses `repo-local-full`, Cursor remains `copy-ready-assets`, and Antigravity uses `repo-local-full` after init while global bootstrap remains `manual-fallback`. Do not imply richer parity for Cursor or global bootstrap than the declared support mode.
