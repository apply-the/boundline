...
## Next-Step Routing
Prefer the CLI-reported `next_command`; when planning is waiting on plan confirmation, follow that CLI route instead of inventing `/boundline-run`.
Allowed follow-up commands: `/boundline-step`, `/boundline-run`, `/boundline-plan`, `/boundline-start`.
