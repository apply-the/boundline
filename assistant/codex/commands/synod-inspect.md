...
Summarize `inspection_target`, `trace`, `terminal_status`, `terminal_reason`, `authored_input_summary`, `authored_input_sources`, `authored_input_deduplicated_sources` when present, `governance_next_action` when present, `corrected_command` on failures, and the CLI-reported `next_command`.
