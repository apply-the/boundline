...
For Codex surfaces, render host-native actions using `/boundline:*` command ids (for example `/boundline:next`).
...
Prefer `assistant_resume_command` when present; otherwise prefer `assistant_next_command`; otherwise render the CLI-reported `next_command` as the primary action. Route to `/boundline:inspect` when the runtime points to inspect, and show `/boundline:status` only when context needs review before acting or `continuity_authority: compatibility_trace` is present. Route to `/boundline:goal` only when the CLI reports no active session and no compatibility follow-up.
