...
**Scale/Scope**: First slice covers the session-native lifecycle commands (`start`, `capture`, `flow`, `plan`, `step`, `run`, `status`, `next`) plus `inspect`, and aligns the assistant command packs that already wrap those surfaces
...
- **PASS** Primary workflow: The main operator path remains session-native (`start -> capture -> plan -> run -> status -> next -> inspect`), and explicit compatibility continuations stay visible rather than implicit. See Summary and Scale/Scope.
...
- **PASS** Tool-agent symmetry: Hosts continue to invoke explicit commands (`start`, `capture`, `plan`, `run`, `status`, `next`, `inspect`) and receive explicit action or inspection data, not hidden heuristics. See Scale/Scope and contract.
