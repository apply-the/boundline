...
- With any human-input flag present, `run` must use the same normalization path as `goal` before planning or continuing execution.
...
- `goal` fails with invalid invocation if neither `--goal` nor `--brief` is supplied.
...
- Existing goal-only usage remains valid for both `goal` and `run`.
