...
**Created**: 2026-04-27  
**Status**: Draft  
**Input**: User description: "Make Synod human-facing by accepting chat text, one or more Markdown briefs, and chat text that references existing Markdown files as external input, so Copilot users can run bug-fix, change, or delivery work without authoring internal JSON manifests."
...
As a developer using Synod from Copilot or the terminal, I want to start a
bounded bug-fix, change, or delivery task from plain text or one authored brief
so that I can use Synod without learning or authoring internal manifests.

**Why this priority**: If Synod cannot start from human-authored input, the
...
brief, verify that Synod captures the request, derives a bounded task draft, and
...
1. **Given** a developer provides a plain-language task request in chat or at
  the CLI, **When** Synod starts the task, **Then** it captures that request as
  the external brief, derives a bounded delivery task, and proceeds without
  requiring a user-authored JSON manifest.
2. **Given** a developer provides one Markdown brief, **When** Synod starts the
...
3. **Given** the provided text or brief is too vague, too broad, or missing a
  critical business constraint, **When** Synod cannot derive a credible bounded
...
brief; value appears only when Synod can ingest the documents humans already
have.

**Independent Test**: Start a task from multiple Markdown files and a text note
that references existing repository documents, verify that Synod resolves them
...
1. **Given** a developer provides more than one Markdown input, **When** Synod
...
2. **Given** a developer provides text that names existing Markdown files in the
  repository, **When** Synod starts the task, **Then** it resolves those files
...
3. **Given** one referenced document is missing, unreadable, duplicated in a
  conflicting way, or contradicts another authored source, **When** Synod starts
...
high-level governance intent alongside my brief so that Synod can use Canon
...
in human terms; otherwise Synod still leaks its internal model even when the
input brief is authored correctly.

**Independent Test**: Start a governed task from human-authored input plus
business-level governance attributes, verify that Synod maps them into internal
...
1. **Given** a developer provides human-authored task input plus governance
  ...
  **When** Synod starts the task, **Then** it applies governance internally and
...
2. **Given** a governed task is missing one required governance attribute,
  **When** Synod cannot continue credibly, **Then** it asks only for the missing
...
3. **Given** Canon blocks the task or requires approval, **When** the developer
  continues through Synod, **Then** Synod remains in an explicit blocked or
...
- If the developer provides both plain text and one or more Markdown sources,
  Synod must preserve which source overrides or supplements another instead of
  blending them silently.
- If the same Markdown document is provided directly and also referenced from
  text, Synod must deduplicate it deterministically and expose the resolved
  provenance.
- If a referenced document path points outside the workspace boundary or to a
  non-text artifact that Synod cannot credibly ingest in the first slice, Synod
  must stop or clarify explicitly.
- If Synod cannot derive a credible flow or bounded validation path from the
...
- If the developer resumes the same session from Copilot after one or more
  clarifications, Synod must preserve the accepted authored inputs and ask only
...
- **FR-001**: Synod MUST accept external task input as direct text, one Markdown
  document, multiple Markdown documents, or direct text that refers to existing
  Markdown documents in the workspace.
- **FR-002**: Synod MUST let developers initiate capture, planning, or run flows
  from that external input without requiring them to author `.synod/execution.json`,
  `.synod/fixture.json`, or any equivalent internal manifest by hand.
- **FR-003**: Synod MUST normalize all accepted external inputs into one bounded,
  inspectable task-input record before planning or execution begins.
- **FR-004**: Synod MUST preserve the provenance of each accepted input source,
  including whether it came from direct text, an attached Markdown document, or
  a referenced workspace document.
- **FR-005**: Synod MUST expose the captured input summary and resolved source set
  through inspectable session or trace surfaces.
- **FR-006**: Synod MUST resolve referenced Markdown documents only from within
...
- **FR-007**: Synod MUST deduplicate repeated input sources deterministically and
  preserve one explicit precedence order when direct text, attached documents,
  and referenced documents overlap.
- **FR-008**: Synod MUST ask targeted clarification questions only when the
  provided human-facing input is insufficient, contradictory, or too broad to
  derive a credible bounded task.
- **FR-009**: When clarification is required, Synod MUST ask only for missing
...
- **FR-010**: Synod MUST support the same external input model for both direct
  CLI usage and assistant-driven usage such as Copilot.
- **FR-011**: Synod MUST allow users to provide governance intent in external
  business terms, including whether governed execution is requested and any
  required risk, zone, or owner values.
- **FR-012**: Synod MUST translate that governance intent into internal
  stage-scoped behavior without requiring the user to author stage mappings or
  runtime-selection configuration.
- **FR-013**: When governed execution becomes blocked or approval-gated, Synod
...
- **External Task Input**: The user-authored task brief presented to Synod,
...
- **Clarification Record**: The explicit missing or conflicting business context
  that Synod must obtain before it can derive a credible bounded task.
...
- **Derived Task Draft**: The inspectable internal task representation Synod
...
- **SC-001**: In representative bug-fix, change, and delivery scenarios,
  developers can start a bounded Synod task from plain text or authored Markdown
  input without manually authoring an internal manifest in 100% of observed runs.
- **SC-002**: In representative multi-source scenarios, Synod resolves the
...
- **SC-004**: Developers can identify what human-authored inputs Synod used,
...
- Synod continues to execute one bounded active session per workspace in the
...
- Developers using this feature already operate inside a repository that Synod
