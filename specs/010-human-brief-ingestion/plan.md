...
**Storage**: Workspace-local `.synod/session.json`, persisted execution traces under `<workspace>/.synod/traces/`, existing `<workspace>/.synod/execution.json` with legacy fallback to `<workspace>/.synod/fixture.json` for advanced automation, and optional Canon-managed artifacts under `<workspace>/.canon/` when governed execution is selected  
