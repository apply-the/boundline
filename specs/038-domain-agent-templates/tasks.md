...
- [x] T001 Confirm and keep synchronized `specs/038-domain-agent-templates/plan.md`, `specs/038-domain-agent-templates/research.md`, `specs/038-domain-agent-templates/data-model.md`, `specs/038-domain-agent-templates/contracts/`, and `specs/038-domain-agent-templates/quickstart.md`
- [x] T002 [P] Add or update top-level test harness references in `tests/unit.rs`, `tests/contract.rs`, and `tests/integration.rs` for new 038 test modules
...
- [x] T003 Extend `src/domain/configuration.rs` and create `src/domain/domain_templates.rs` with the domain-family catalog, scoped template settings, external binding model, effective resolution helpers, and validation rules
- [x] T004 [P] Extend `src/cli.rs`, `src/cli/config.rs`, and `src/cli/init.rs` with the request/command primitives needed to seed, mutate, and render domain-template settings across workspace, cluster, and global scopes
- [x] T005 [P] Extend `src/domain/goal_plan.rs`, `src/domain/task_context.rs`, and `src/orchestrator/goal_planner.rs` so context packs can carry applied domain context, supporting-input status, and blocked-domain reasons
- [x] T006 [P] Add foundational coverage for domain-template validation and scoped resolution in `tests/unit/domain_templates.rs` and `tests/contract/domain_template_configuration_contract.rs`
...
- [x] T007 [P] [US1] Add contract coverage for domain-template config rendering in `tests/contract/domain_template_configuration_contract.rs`
- [x] T008 [P] [US1] Add integration coverage for workspace init and domain-family selection in `tests/integration/domain_template_init_flow.rs` and `tests/integration/domain_template_context_flow.rs`
- [x] T009 [P] [US1] Add unit coverage for file-target-to-domain selection in `tests/unit/domain_templates.rs` and `tests/unit/goal_planner_domain_context.rs`

### Implementation for User Story 1

- [x] T010 [US1] Extend `src/cli/init.rs` and `src/cli.rs` so `boundline init` can detect or accept active domain families and seed workspace-scoped domain settings
- [x] T011 [US1] Extend `src/orchestrator/goal_planner.rs` and `src/domain/goal_plan.rs` so planning selects a credible applied domain context from task targets and blocks explicitly when no matching active family exists
- [x] T012 [US1] Extend `src/cli/config.rs`, `src/cli/output.rs`, and `src/cli/inspect.rs` so `config show`, `plan`, and `inspect` expose the active domain family, selected target, and winning guidance source
...
- [x] T013 [P] [US2] Add integration coverage for shared-vs-workspace standards precedence in `tests/integration/domain_template_standards_inheritance.rs`
- [x] T014 [P] [US2] Add unit coverage for precedence and source attribution in `tests/unit/domain_templates.rs` and `tests/unit/configuration_domain_resolution.rs`

### Implementation for User Story 2

- [x] T015 [US2] Extend `src/domain/configuration.rs` and `src/cli/config.rs` so operators can enable or disable domain families and update scoped standards after initialization
- [x] T016 [US2] Extend `src/domain/domain_templates.rs`, `src/orchestrator/goal_planner.rs`, and `src/cli/output.rs` so effective domain guidance resolves in explicit built-in → shared → workspace precedence order and remains inspectable
- [x] T017 [US2] Extend `src/cli/session.rs` and `src/cli/inspect.rs` so `status` and `inspect` keep the layered-domain story visible after later replans or task-target changes
...
- [x] T018 [P] [US3] Add integration coverage for blocked-domain planning and mixed-stack replanning in `tests/integration/domain_template_context_flow.rs`
- [x] T019 [P] [US3] Add contract coverage for domain-context projection on inspection surfaces in `tests/contract/domain_template_context_contract.rs`

### Implementation for User Story 3

- [x] T020 [US3] Extend `src/orchestrator/goal_planner.rs`, `src/orchestrator/decision_loop.rs`, and `src/orchestrator/session_runtime.rs` so domain context can be recomputed from bounded targets and recorded when the current target changes
- [x] T021 [US3] Extend `src/domain/task_context.rs`, `src/domain/trace.rs`, `src/cli/output.rs`, `src/cli/inspect.rs` so applied-domain context, blocked-domain reasons, and context credibility stay authoritative and inspectable
...
- [x] T022 [P] [US4] Add contract coverage for governed artifacts and external binding status in `tests/contract/external_context_binding_contract.rs`
- [x] T023 [P] [US4] Add integration coverage for Canon-governed and external input reuse in `tests/integration/domain_template_external_inputs.rs`
- [x] T024 [P] [US4] Add unit coverage for binding availability and required-input blocking in `tests/unit/domain_templates.rs` and `tests/unit/goal_planner_domain_context.rs`

### Implementation for User Story 4

- [x] T025 [US4] Extend `src/domain/domain_templates.rs`, `src/domain/configuration.rs`, and `src/cli/config.rs` so operators can bind and unbind optional or required external context inputs per family and scope
- [x] T026 [US4] Extend `src/orchestrator/goal_planner.rs`, `src/domain/goal_plan.rs`, and `src/domain/task_context.rs` so Canon-governed artifacts and external inputs augment the applied domain context with explicit status
- [x] T027 [US4] Extend `src/cli/output.rs`, `src/cli/session.rs`, and `src/cli/inspect.rs` so `plan`, `run`, `status`, `next`, and `inspect` surface supporting-input status without implying those systems own template selection
...
- [x] T036 Mark completed tasks in `specs/038-domain-agent-templates/tasks.md` and capture the final descriptive commit message in the implementation summary
