...
2. Run `boundline init --workspace <workspace> --assistant copilot`.
3. Confirm `.boundline/config.toml` exists and shows Copilot plus auto-seeded route defaults.
4. Run `boundline doctor --workspace <workspace>`.
...
2. Run `boundline doctor --workspace <workspace>`.
3. Run `boundline run --workspace <workspace> --goal "Bootstrap the repository"`.
...
1. Run `boundline init --workspace <workspace> --assistant claude` in a fresh workspace.
...
2. Run `boundline init --workspace <workspace> --domain react --assistant codex`.
