...
**Created**: 2026-04-24  
**Status**: Draft  
**Input**: User description: "Expose Synod workflows as assistant-native slash commands for Copilot, Codex, and Claude, instead of relying on long CLI commands."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Start a Workflow from Chat (Priority: P1)

A developer working in a chat-first assistant environment can start a Synod workflow through a short assistant-native command and receive the exact next action without memorizing CLI syntax.
...
**Independent Test**: Can be fully tested by invoking the start or plan command in a supported assistant environment with and without direct shell access, and confirming the user receives a clear path to begin a Synod workflow.

**Acceptance Scenarios**:

1. **Given** a developer is using a supported assistant environment and has not started a Synod workflow, **When** they invoke the planning command from chat, **Then** the assistant asks only for the missing goal information, provides or performs the correct next action, and explains what will happen next.
...
A developer can continue an existing Synod workflow from chat, run the next bounded action, and receive a concise summary of progress, failures, and next steps.
...
1. **Given** a Synod workflow is in progress, **When** the developer invokes a run, step, status, or next command from chat, **Then** the assistant executes or guides the requested action and responds with a summary of the current state, recent result, and recommended follow-up.
...
A developer can inspect a completed or failed Synod run from chat and understand what happened without manually reading raw trace output.
...
- The underlying Synod action fails, retries, replans, or reaches a terminal limit, and the assistant must explain the outcome without dumping raw logs.
...
- **FR-001**: The system MUST provide an assistant-native command set that lets users start, continue, inspect, and get next-step guidance for Synod workflows from supported assistant environments.
...
- **FR-006**: The system MUST provide a consistent command experience across the supported assistant environments so that the same Synod workflow can be started and continued with equivalent user guidance.
...
- **FR-008**: The system MUST remain compatible with Synod's CLI-first workflow model and MUST not require a new external runtime service or one-to-one new Synod CLI subcommands in order for assistant-native commands to function.
...
- **Out of Scope**: Replacing the existing CLI, adding one-to-one Synod CLI subcommands solely to mirror assistant command names, introducing new orchestration logic, building a general plugin marketplace, adding platform-specific APIs or servers, or expanding into broader UI or deployment work.
...
- **Workflow Command Definition**: A single assistant-facing command that captures the action name, required inputs, execution path, fallback path, and summary expectations for one Synod workflow step.
...
- **Inspection Summary**: A user-readable explanation of what happened during a Synod run, including final state, meaningful recovery events, and the next recommended action.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: In usability checks, first-time users can start a Synod workflow from a supported assistant environment in under 2 minutes without consulting separate CLI documentation.
...
- The existing Synod CLI remains the source of truth for workflow execution and inspection behavior during the initial release of this feature.
