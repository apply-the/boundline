...
As a developer using Synod for the first time in a repository, I want a guided
`synod init` flow that prepares the workspace for bounded delivery work so that
...
Synod remains human-hostile at the exact moment a new operator decides whether
to trust it.

**Independent Test**: In a repository that does not already contain Synod
workspace files, run `synod init`, choose a delivery template, confirm the
...
1. **Given** a repository with no `.synod` workspace files, **When** the
  developer runs `synod init` and selects a bounded template such as bug-fix,
  change, or delivery, **Then** Synod creates the required workspace files,
  explains what was created, and leaves the repository ready for the normal
  session flow.
2. **Given** a repository that already contains Synod workspace files,
  **When** the developer reruns `synod init`, **Then** Synod shows what already
...
3. **Given** the selected setup cannot continue because the repository lacks a
  ...
  reaches that point, **Then** Synod stops explicitly, preserves the current
...
As a developer installing Synod globally or across multiple repositories, I
...
1. **Given** a developer has global Synod defaults and no workspace override,
  **When** Synod resolves routing for a supported step, **Then** it uses the
...
2. **Given** a workspace overrides only some routing values, **When** the
  developer inspects the effective configuration, **Then** Synod shows the
...
3. **Given** the developer tries to save an invalid runtime/model combination
  or a configuration that references an unavailable runtime, **When** Synod
...
As a developer relying on Synod review and governance flows, I want different
...
adjudicator profile; then verify that Synod reports and uses the correct
effective routing for each role.

**Acceptance Scenarios**:

1. **Given** a developer configures different defaults for planning,
  implementation, verification, review, and adjudication, **When** Synod
...
2. **Given** a voting council contains multiple reviewer roles, **When** the
  developer configures review routing, **Then** Synod allows reviewer roles to
...
3. **Given** the developer selects a runtime that needs repository-local prompt
  or command-pack setup, **When** `synod init` or a later config command applies
  that choice, **Then** Synod offers to create or refresh the repository-local
...
- If no supported runtime is available on the machine, Synod must still let the
...
- If the developer selects Gemini, Synod must treat it as a CLI-only runtime in
  the first slice and must not imply that a richer native client integration is
  available.
- If a workspace override deletes or conflicts with a global value, Synod must
  resolve the effective configuration deterministically and explain which value
  won.
- If `synod init` encounters existing config files that differ from the chosen
  template, Synod must preview the change and require an explicit confirmation
  before overwriting or merging.
- If a review council is configured with duplicated reviewer identities or a
  missing adjudicator route, Synod must stop with actionable guidance rather
  than silently collapsing to one default model.
- If a repository-local assistant setup step would modify files outside the
  active repository root, Synod must refuse that change.
...
- **FR-001**: Synod MUST provide a `synod init` workflow that can prepare a
  repository for bounded delivery work without requiring the operator to
  hand-author internal JSON files.
- **FR-002**: Synod MUST let the operator choose an initial bounded workspace
  template appropriate for bug-fix, change, or delivery work during init.
- **FR-003**: Synod MUST present setup choices and explanations in user-facing
  terms rather than exposing internal nouns that only make sense to the
  implementation.
- **FR-004**: Synod MUST support Claude, Codex, Copilot, and Gemini as runtime
  choices in the first slice, with Gemini explicitly treated as CLI-only.
- **FR-005**: Synod MUST detect which supported runtimes are currently usable
...
- **FR-006**: If a selected runtime needs repository-local support files,
  command packs, or prompts, Synod MUST offer to scaffold or refresh those
  files as part of init or a later setup command.
- **FR-007**: Synod MUST persist user-scoped default configuration separately
  from workspace-local configuration so a global installation can supply
  defaults while repositories can override them.
- **FR-008**: Synod MUST resolve effective configuration using this precedence:
  explicit CLI input first, then workspace-local config, then user-scoped global
  config, then built-in defaults.
- **FR-009**: Synod MUST let operators inspect the effective resolved config,
  including the source of each resolved value.
- **FR-010**: Synod MUST let operators change saved configuration later through
  CLI commands instead of requiring manual file editing.
- **FR-011**: Synod MUST support separate runtime/model defaults for at least
  planning, implementation, verification, review, and adjudication stages.
- **FR-012**: Synod MUST support review-role-specific routing so multiple
  reviewer roles in a voting council can intentionally use different runtime or
  model defaults from one another.
- **FR-013**: Synod MUST allow the adjudicator to use a distinct routing profile
  from the main review council.
- **FR-014**: Synod MUST validate provider/model assignments before saving or
  applying them and MUST refuse invalid or unavailable combinations with clear,
  actionable feedback.
- **FR-015**: Synod MUST show a preview or summary of the files and config it is
...
- **FR-016**: Rerunning init on an already-configured repository MUST preserve
  ...
  merge them.
- **FR-017**: Synod MUST keep the existing advanced manifest-driven path
  available for automation and expert use, while making guided init the normal
  human-facing entry point.
- **FR-018**: Synod MUST emit inspectable status or trace output for init and
  routing decisions so developers can understand what setup occurred and why a
  value was chosen.
- **FR-019**: Synod MUST stop in an explicit terminal state when setup cannot
...
- **FR-020**: Synod MUST document the feature across the main README, workflow
...
- **Workspace Init Profile**: The user-facing setup choice that defines the
  ...
  Synod files for one repository.
...
- **SC-001**: In representative first-run repositories, developers can reach a
  usable Synod workspace through `synod init` in under 5 minutes without
...
- **SC-004**: In representative rerun and failure scenarios, Synod reaches an
...
- Synod continues to operate as a bounded CLI-first workflow and does not gain a
  separate GUI in this feature.
- Supported runtimes may differ in how they are invoked, but Synod can still
  describe them through one user-facing routing model.
- Operators may install Synod globally, but repository-local overrides remain
