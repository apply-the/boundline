...
Define how Boundline resolves effective runtime/model routing when values exist at
multiple scopes and how the CLI explains that resolution to the operator.

## Resolution Precedence

Boundline resolves every supported routing slot independently with this precedence:
...
- If no saved value exists at any scope, Boundline must show the built-in default
  rather than a blank or implied route.

## Failure Rules

- If a higher-precedence value is invalid, Boundline must fail explicitly instead of
...
- If duplicate reviewer-role routes exist at one scope, Boundline must reject that
  scope as invalid rather than guess which one wins.

## Output Rules

- `boundline config show --scope effective` must render one human-readable table or
