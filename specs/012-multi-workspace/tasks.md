...
**Goal**: Let operators register a bounded cluster of related workspaces through an explicit Boundline entry point.

**Independent Test**: Register two valid workspaces into one cluster, verify `.boundline/cluster.toml` is created in the primary workspace, and confirm invalid or duplicate members fail without partial state.
...
Task: "T009 Add contract coverage for boundline cluster init membership validation and failure behavior in tests/contract/cluster_cli_contract.rs"
...
Task: "T013 Implement boundline cluster init command flow in src/cli/cluster.rs and src/cli.rs"
```
