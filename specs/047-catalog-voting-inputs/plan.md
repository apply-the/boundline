...
**Execution Model**: Sequential session-native lifecycle (`start -> capture -> plan -> run -> status -> next -> inspect`) plus explicit compatibility capture/run paths; review steps remain one reviewer at a time followed by one bounded vote resolver  
...
- **PASS** Primary workflow: The primary path remains session-native (`start -> capture -> plan -> run -> status -> next -> inspect`), while compatibility behavior stays explicit in the shared capture/run input normalization path. See Technical Context and research decisions.
