...
**Refinement Note**: Checked tasks capture the baseline host, template, and Speckit work that still applies after the V1 contract clarification. Follow-up tasks T042-T047 absorb the refined success or error envelope, `describe.supported_transports`, stderr trace-only handling, and the explicit one-shot stdio-only scope without reopening already honest completions. Follow-up tasks T048-T051 close the remaining ownership gap by making declared `plan` and `run` overrides authoritative stage paths and by upgrading the Speckit adapter from placeholder claimed-stage markers to a real Speckit workflow bridge.
...
- [X] T050 [P] [US2] Add Speckit repo end-to-end validation that claimed stages invoke real Speckit workflow entrypoints, return real produced artifacts or blocked outcomes, and fail acceptance when only placeholder markers are produced in `../boundline-adapter-speckit/tests/override_flow.rs`, `../boundline-adapter-speckit/tests/contract.rs`, and `../boundline-adapter-speckit/README.md`.
- [X] T051 [US2] Replace the Speckit placeholder claimed-stage path with a real Speckit workflow bridge that consumes host context, invokes the appropriate Speckit command for `plan` and `run`, and returns authoritative produced_artifacts or blocked/failure outcomes in `../boundline-adapter-speckit/src/main.rs`, `../boundline-adapter-speckit/src/profile.rs`, `../boundline-adapter-speckit/src/stages.rs`, `../boundline-adapter-speckit/src/hooks.rs`, and `../boundline-adapter-speckit/src/config.rs`.
...
- [X] T035 [P] Update host operator docs and validation guidance for adapter registration, supported transport inspection, standard response-envelope semantics, optional structured stderr, and the V1 one-shot stdio scope with its no-graceful-shutdown boundary in `README.md`, `docs/configuration.md`, `docs/getting-started.md`, `docs/architecture.md`, and `specs/066-agentic-framework-integration/quickstart.md`.
- [X] T036 [P] Update host wiki pages for adapter registration, configuration, and runtime inspection in `../boundline.wiki/Home.md`, `../boundline.wiki/Getting-Started.md`, `../boundline.wiki/Configuration-Reference.md`, `../boundline.wiki/Architecture-And-Decisions.md`, and `../boundline.wiki/Reference.md`.
- [X] T037 Update host release metadata and changelog for the adapter compatibility line in `Cargo.toml`, `distribution/channel-metadata.toml`, `distribution/homebrew/Formula/boundline.rb`, and `CHANGELOG.md`.
- [ ] T038 [P] Update template repo release docs, released `boundline-adapters` dependency pins, and compatibility notes for the standard envelope, declared V1 stdio JSON transport, optional structured stderr as trace-only enrichment, and no graceful-shutdown expectation in `../boundline-framework-template/README.md`, `../boundline-framework-template/Cargo.toml`, and `../boundline-framework-template/CHANGELOG.md`.
- [ ] T039 [P] Update Speckit repo release docs, released `boundline-adapters` dependency pins, and compatibility notes for the standard envelope, declared V1 stdio JSON transport, optional structured stderr as trace-only enrichment, and no graceful-shutdown expectation in `../boundline-adapter-speckit/README.md`, `../boundline-adapter-speckit/Cargo.toml`, and `../boundline-adapter-speckit/CHANGELOG.md`.
- [X] T040 Update completed feature tracking and roadmap status in `roadmap/features/02-agentic-framework-integration.md`, `roadmap/Next - forward-roadmap.md`, and `specs/066-agentic-framework-integration/tasks.md`.
- [X] T041 Run cross-repo validation, capture compatibility evidence for the released protocol line, supported transport declarations, standard response-envelope fixtures, unsupported-transport blocking, optional structured stderr remaining trace-only, and the explicit V1 no-graceful-shutdown scope, and record the provider-catalog refresh no-change result in `specs/066-agentic-framework-integration/research.md`, `specs/066-agentic-framework-integration/quickstart.md`, `CHANGELOG.md`, `../boundline-framework-template/README.md`, and `../boundline-adapter-speckit/README.md`.
