...
- [X] T001 Bump crate version to `0.30.0` in `Cargo.toml` and `Cargo.lock`
- [X] T002 Reuse the existing runtime-routing and direct-run test harness surfaces in `tests/contract/runtime_routing_contract.rs`, `tests/contract/compatibility_continuity_contract.rs`, `tests/integration/runtime_refoundation_compat.rs`, `tests/integration/fixture_compat_flow.rs`, `tests/integration/session_compatibility_continuity.rs`, and `tests/unit/coverage_additional.rs`
...
- [X] T003 Add explicit direct-run route selection inputs to `src/cli.rs` and `src/cli/run.rs`
- [X] T004 [P] Add native direct-run bootstrap helpers that can create safe executable session state in `src/cli/run.rs` and `src/cli/session.rs`
- [X] T005 [P] Split native-ready versus compatibility-ready diagnostics in `src/cli/diagnostics.rs`
- [X] T006 [P] Add foundational unit coverage for run validation and route selection changes in `tests/unit/coverage_additional.rs` and `tests/unit/cli_output.rs`
...
- [X] T007 [P] [US1] Add contract and integration coverage for native direct-run routing in `tests/contract/runtime_routing_contract.rs` and `tests/integration/runtime_refoundation_compat.rs`
- [X] T008 [P] [US1] Add follow-up continuity coverage for direct native run in `tests/integration/session_compatibility_continuity.rs` and `tests/integration/fixture_compat_flow.rs`

### Implementation for User Story 1

- [X] T009 [US1] Bootstrap native session state for direct `run --goal` in `src/cli/run.rs`
- [X] T010 [US1] Make direct native run produce an executable route by confirming inferred flows or choosing no-flow planning in `src/cli/run.rs` and `src/orchestrator/session_runtime.rs`
- [X] T011 [US1] Reuse the persisted native session story on direct run output and later follow-up surfaces in `src/cli/run.rs`, `src/cli/session.rs`, and `src/domain/session.rs`
...
- [X] T012 [P] [US2] Add contract coverage for explicit compatibility opt-in and active-session protection in `tests/contract/runtime_routing_contract.rs` and `tests/contract/compatibility_continuity_contract.rs`
- [X] T013 [P] [US2] Add integration coverage for direct-run session-protection and explicit compatibility behavior in `tests/integration/runtime_refoundation_compat.rs` and `tests/integration/session_native_flow.rs`

### Implementation for User Story 2

- [X] T014 [US2] Add an explicit compatibility opt-in surface for `run` in `src/cli.rs` and `src/cli/run.rs`
- [X] T015 [US2] Block destructive direct-run bootstrap when meaningful active session state already exists in `src/cli/run.rs` and `src/adapters/session_store.rs`
- [X] T016 [US2] Keep compatibility ownership and diagnostics explicit after the route default changes in `src/cli/run.rs`, `src/cli/diagnostics.rs`, and `src/cli/session.rs`
...
- [X] T017 [P] [US3] Extend assistant and command-surface contract coverage for native direct run and explicit compatibility opt-in in `tests/contract/assistant_command_definition_contract.rs` and `tests/contract/assistant_session_continuity_contract.rs`
...
- [X] T020 Run focused coverage for modified or created Rust files, refresh `lcov.info`, verify touched-Rust coverage remains above 95%, resolve remaining `cargo clippy` issues, run `cargo fmt --all`, and finish with clean validation for touched files under `src/` and `tests/`
