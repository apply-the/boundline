...
Define how Synod preserves the execution-profile compatibility route without
...
- Compatibility execution must occur only when the operator chooses it
  deliberately.
- When compatibility execution is chosen, Synod must keep routing, ownership,
...
- This contract does not delete `.synod/execution.json` support.
