...
- **PASS** Primary workflow: The primary operator path remains session-native (`start -> capture -> plan -> run -> status -> next -> inspect`), while any compatibility path remains explicit and secondary in the same runtime surfaces. See Summary, Technical Context, and `quickstart.md`.
