...
**Decision**: Extend Boundline's Canon governance surface with `security-assessment` in the first slice and explicitly defer real `supply-chain-analysis` support and the rest of Canon's unsupported mode roster.

**Rationale**: `security-assessment` is the newest governance-hardened Canon mode that can deepen Boundline's delivery-facing governance without introducing the larger clarification and tool-availability UX implied by `supply-chain-analysis`. It adds immediate value while preserving the smallest independently useful slice.
...
**Decision**: Keep Boundline's built-in `bug-fix`, `change`, and `delivery` flows unchanged and extend the targeted verification stages so they may route through `security-assessment` as a governed Canon mode.
...
**Alternatives Considered**:
- Add new top-level Boundline flow stages for security analysis: rejected because it expands the flow model before the underlying governance model needs it.
...
**Decision**: Keep Boundline's Canon integration anchored on the current `governance start` and `governance refresh` request/response contract.

**Rationale**: Boundline already performs real Canon start and refresh calls, packet-readiness validation, and approval refresh. The next slice should widen what those calls can represent, not add a second governance protocol or hidden background behavior.
...
**Rationale**: The governed analysis packet should improve downstream reasoning without letting later steps inspect or depend on the full `.canon/` artifact tree. This matches Boundline's existing bounded-context posture and keeps the flow inspectable.
...
**Decision**: Update Boundline's documented Canon compatibility target to `0.25.0` while explicitly basing the governance-expansion design on the released Canon mode and authoring changes from `0.20.0` through `0.24.0`.

**Rationale**: Canon `0.25.0` primarily adds distribution-channel work. The governance-relevant feature delta that motivates this Boundline slice is the newer governed analysis and authoring surface added between `0.20.0` and `0.24.0`.

**Alternatives Considered**:
- Treat `0.25.0` itself as the governance feature: rejected because it does not add a materially new governance runtime capability for Boundline.
- Leave the compatibility target at `0.24.0`: rejected because the user's environment already relies on Canon `0.25.0` and Boundline should document the validated target honestly.