...
- [x] T013 Add CLI snapshot and contract tests for the M1C command-classification model in `boundline:tests/contract/cli_090.rs`. Verify the current StableOperational subset only; migration diagnostics and immediate removal for `orchestrate`, `step`, `continue`, `next`, `probe`, `help-next`, and `govern`; the M1C `govern` diagnostic recommends only operational `plan` and `run`, with `approve` added only after T037 promotes it to StableOperational; preview placement for `flow`, `workflow`, `cluster`, `council`, `evals`, `override`, and `trace`; internal hiding for `checkpoint` and `exec`; and absence of every StableTargetPending command from parser registration, stable help, and completion metadata.
- [x] T014 Replace the two Boundline CLI definitions with the M1C 0.90 command tree in `boundline:src/cli.rs` and `boundline:crates/boundline-cli/src/cli.rs`: implement only the explicit command-classification model, the currently operational StableOperational subset, removed-command migration diagnostics, preview placement, internal hiding, and consistent stable help/completions. The M1C `govern` diagnostic MUST recommend only `plan` and `run`; add `approve` only when T037 promotes it to StableOperational. Do not register `approve`, `session abort`, `session cleanup`, `recover`, `rpc`, or `serve --transport mcp-stdio`; do not add placeholder handlers or generic not-implemented responses.
...
- [x] T020 Publish and verify the exact reviewed prerelease package candidates, then create and verify matching signed-tag provenance metadata as specified in `boundline:tech-docs/release-checklist.md`, `canon:CHANGELOG.md`, and `evidence/m1e-package-readiness.md`. The immutable completion evidence records crates.io retrieval, registry-only consumers, signed tag objects, and isolated remote verification; T019 itself performed no publication.
...
- [ ] T095 [US5] Implement the one-shot Boundline `rpc` runtime handler and typed request/response dispatch in `boundline:crates/boundline-cli/src/commands/rpc.rs`, add fail-closed transport and authority contract tests in `boundline:tests/contract/rpc.rs`, and update promotion documentation in `boundline:specs/083-governed-1-0-release/contracts/boundline-protocol-v1.md`. Prerequisites: T037, T048, T061, and T073. Acceptance: no request is inferred as complete or authorized, one bounded request is processed per invocation, the contract suite passes, and the same commit promotes `rpc` from StableTargetPending by updating parser registration, stable help, completion metadata, and documentation.
- [ ] T096 [US5] Implement bounded stdio framing, fail-closed capability and authority semantics, and process cleanup for `serve --transport mcp-stdio` in `boundline:crates/boundline-cli/src/commands/serve.rs`, add MCP stdio contract tests in `boundline:tests/contract/mcp_stdio.rs`, and update promotion documentation in `boundline:specs/083-governed-1-0-release/contracts/boundline-protocol-v1.md` and `boundline:specs/083-governed-1-0-release/host-compatibility.md`. Prerequisites: T037, T048, T061, and T073. Acceptance: the MCP stdio handler is real and bounded, unsupported transports fail closed, the contract suite passes, and the same commit promotes the command from StableTargetPending by updating parser registration, stable help, completion metadata, and documentation.
- [ ] T076 [US5] Generate and validate Claude, Codex, Copilot, Cursor, and Antigravity compatibility packs against the frozen host matrix in `boundline:assistant/` and `specs/083-governed-1-0-release/host-compatibility.md`; depends on T095 and T096 because host qualification consumes operational RPC and MCP stdio projections rather than creating their runtime handlers.
...
- T076 additionally depends on T095 and T096. T095 owns RPC runtime creation and T096 owns MCP stdio runtime creation; T076 only qualifies their host and projection behavior.
