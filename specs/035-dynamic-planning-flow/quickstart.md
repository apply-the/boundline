...
1. Review the proposed plan with `synod status --workspace <repo>` or
   `synod inspect --workspace <repo>`.
2. Confirm the current proposal with `synod plan --workspace <repo> --confirm`.
3. Run `synod run --workspace <repo>`.
...
1. After a failed verification or a new trace/context input, run
   `synod plan --workspace <repo> --replan`.
...
3. Re-confirm with `synod plan --workspace <repo> --confirm` before running
   again.
...
2. Run `synod plan --workspace <repo>`.
