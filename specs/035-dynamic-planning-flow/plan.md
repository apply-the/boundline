...
**Branch**: `035-dynamic-planning-flow` | **Date**: 2026-05-02 | **Spec**: [/Users/rt/workspace/synod/specs/035-dynamic-planning-flow/spec.md](/Users/rt/workspace/synod/specs/035-dynamic-planning-flow/spec.md)
...
Replace Synod's keyword-first flow inference and stage-static goal planning with
...
evidence. Keep the operator-facing CLI surface bounded by making `synod plan`
produce a proposal, `synod plan --confirm` confirm the current proposal, and
`synod plan --replan` supersede the prior proposal with a bounded revision when
...
**Observability Surface**: Persisted goal-plan/session state, decision-oriented traces under `.synod/traces/`, CLI summaries on `plan`, `run`, `status`, `next`, and `inspect`, plus release docs and assistant guidance that explain proposal, confirmation, and replanning state  
...
- **PASS** Delivery identity: The slice directly changes how Synod derives and controls bounded engineering work by making planning evidence-driven and operator-confirmed. See Summary and Technical Context.
